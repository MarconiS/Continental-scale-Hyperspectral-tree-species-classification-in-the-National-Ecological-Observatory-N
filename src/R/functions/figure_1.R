figure_1 <- function(pairs, data){
  ...
  dir.create("./outdir/cm/general/")
  dir.create("./outdir/cm/siteonly/")
  ...
  write_csv( ccmm, "outdir/site_only.csv")
  ...
  trends_comparison = rbind.data.frame(data.frame(Delta = unlist(macroF1_all)-unlist(macroF1_idst),
                                                  Site = substr(names(macroF1_all),2,5), Score = "MacroF1"),
                                       data.frame(Delta = unlist(microF1_all)-unlist(microF1_idst),
                                                  Site = substr(names(microF1_all),2,5), Score = "MicroF1")
  )


  ntaxa = data.frame(Site = substr(names(sp_trained_in_site),2,5),
