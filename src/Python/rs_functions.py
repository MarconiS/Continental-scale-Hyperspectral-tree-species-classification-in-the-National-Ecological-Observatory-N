...
# -*- coding: utf-8 -*-
"""
Created on Wed Dec 15 16:31:08 2021
...
def calculate_geom_kernel(df, sns_zn = 0, ross = "thick", li = "dense"):
    ...
    solar_zn_ = np.arctan(10*np.tan(df["sol_az"]))
...
def extract_hsi_and_brdf_data(full_path, epsg, ras_dir, year,site, ross="thick", li="dense"):
    ...
    #
    tmp = elevation.reshape([elevation.shape[0],slope.shape[1],1])
    output_df = np.concatenate([hcp, tmp], -1)
    tmp = aspect.reshape([aspect.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    tmp = slope.reshape([slope.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    tmp = sns_zn.reshape([sns_zn.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    tmp = sns_az.reshape([sns_az.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    tmp = sol_zn.reshape([sol_zn.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    tmp = sol_az.reshape([sol_az.shape[0],slope.shape[1],1])
    output_df = np.concatenate([output_df, tmp], -1)
    #save hcp into a tiff file [reflectance]
    itc_id =  str(int(year)) + "_" +site+"_" + str(int(xmin)) + "_" + str(int(ymin)) 
    ii = str(itc_id + ".tif")
    #array2raster(str(ii), output_df, refl_md, clipExtent, ras_dir = str(ras_dir), epsg = int(refl_md['epsg']))
    # output the dataframe
    output_df = output_df.reshape(-1,output_df.shape[2])
    output_df = pd.DataFrame(output_df)
    cl_nm = ["band_"+ str(i).zfill(1) for i in range(1,368)]
    output_df.columns = cl_nm + ["CHM", "aspect","slope","sns_zn","sns_az","sol_zn", "sol_az"]
    return(output_df, refl_md)




def kld_transform(brick):
    kld_groups = pd.read_csv("/blue/ewhite/s.marconi/NeonSpeciesClassification/data/kld_grps_fin.csv", header=None)
