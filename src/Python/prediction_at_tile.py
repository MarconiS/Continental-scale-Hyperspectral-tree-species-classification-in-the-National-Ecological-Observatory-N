...
transform = from_origin(clipExtent['xMin'], clipExtent['yMin'], 1, 1)
outpred = np.moveaxis(outpred.squeeze(),-1,0) # move axis with 4 entries to beginning and remove extra dimension
ras_pt = str(ras_dir)+'/predictions_raster/'+ii
new_dataset = rasterio.open(ras_pt, 'w', driver='GTiff',
                            height = outpred.shape[1], width = outpred.shape[2],
                            count=outpred.shape[0], dtype=str(outpred.dtype),
                            crs=int(refl_md['epsg']),
                            transform=transform)

new_dataset.write(outpred)
new_dataset.close()
