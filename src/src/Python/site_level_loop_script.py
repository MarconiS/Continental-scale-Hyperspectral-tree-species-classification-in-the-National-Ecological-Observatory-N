...
for siteID in domainid.keys():
    ...
    if y_res.taxonID.unique().shape[0]>1:
        ...
        logc = LogisticRegression(penalty = "elasticnet", solver = "saga", 
                                  max_iter = 10000, n_jobs=3, l1_ratio = 0.1)
        ...
        #test_families.to_csv("./outputs/"+siteID+"_"+"family_predictions.csv")   
        cm_fam = confusion_matrix(obs, pred)
        cm_fam = pd.DataFrame(cm_fam, columns = rep_fam.index[0:-3], index = rep_fam.index[0:-3])
        #eval_an.to_csv("./outputs/"+siteID+"_"+"kld_probabilities.csv")
        #pd.DataFrame(np.c_[eval_an[['individualID','taxonID']], pred_itc]).to_csv("./outputs/"+"_"+siteID+"_"+"kld_pairs.csv")
        #pd.DataFrame(taxa_classes).to_csv("./outputs/taxonID_dict_"+"_"+siteID+".csv")
